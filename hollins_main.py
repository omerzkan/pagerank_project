...
x, k, diff, _ = power_method(mv, n, m=M_VAL, tol=TOL)
elapsed = time.perf_counter() - t0

# residual ||Mx - x||_1 : an independent check, NOT the stopping criterion
res = np.abs((1.0 - M_VAL) * mv(x) + M_VAL / n - x).sum()
...
assert x.min() >= M_VAL / n - 1e-12, "every page scores at least m/n [Exercise 9]"
print(f"sum(x) = {x.sum():.12f}   min(x) = {x.min():.3e}   (m/n = {M_VAL / n:.3e})")
