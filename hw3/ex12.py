...
xM, k, _, _ = power_method(lambda v: A6 @ v, EX12_N, m=M_VAL, tol=TOL)   # formula (3.2)

print(f"\nm = {M_VAL},  M-ranking converged in {k} iterations\n")
...
print(f"page 6 with M: {xM[5]:.6f}   -> exactly m/n = {M_VAL / EX12_N:.6f}  [Exercise 9]")
...
x5, _, _, _ = power_method(lambda v: A5 @ v, EX11_N, m=M_VAL, tol=TOL)

ratio = xM[:5] / x5
print("\nExercise 12 score / Exercise 11 score, for pages 1-5:")
print(np.round(ratio, 10))
print(f"all equal, and the common value is 39/40 = {39 / 40}")
